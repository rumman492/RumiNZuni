...
4. Optional catalog seed — two sample products per category (Boys, Girls, Newborn, Unisex) with photos. Replace them later with real stock. Live Docker also loads this catalog on first boot when the shop is empty. Does not create an admin unless `SEED_ADMIN_EMAIL` and `SEED_ADMIN_PASSWORD` are in `.env`:
